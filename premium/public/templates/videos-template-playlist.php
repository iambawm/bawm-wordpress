...
$playlist_position = ( 'bottom' == $attributes['playlist_position'] ) ? 'bottom' : 'right';
$playlist_color = ( 'light' == $attributes['playlist_color'] ) ? 'light' : 'dark';
$playlist_width = ! empty( $attributes['playlist_width'] ) ? (int) $attributes['playlist_width'] : 250;
$playlist_height = ! empty( $attributes['playlist_height'] ) ? (int) $attributes['playlist_height'] : 250;
...
<style type="text/css">
    <?php
    $styles = "#aiovg-" . $uid . " .aiovg-section-videos {
        max-height: " . $playlist_height . "px;
    }";

    if ( 'right' == $playlist_position ) {
        $styles .= "@media only screen and (min-width: 768px) {
            #aiovg-" . $uid . " .aiovg-section-player {
                width: calc(100% - " . $playlist_width . "px);
            }

            #aiovg-" . $uid . " .aiovg-section-videos {
...
<div id="aiovg-<?php echo esc_attr( $uid ); ?>" class="aiovg aiovg-videos aiovg-videos-template-playlist" data-params='<?php echo wp_json_encode( $data_params ); ?>'>
	<?php                    
    // Title
    ...
    <!-- Playlist -->
    <div class="aiovg-playlist aiovg-playlist-<?php echo esc_attr( $playlist_position ); ?> aiovg-playlist-<?php echo esc_attr( $playlist_color ); ?>">
        <!-- Player -->
        ...
        <div class="aiovg-section-videos">
            ...
            while ( $aiovg_query->have_posts() ) :        
                ...
                $image_alt = ! empty( $image_data['alt'] ) ? $image_data['alt'] : get_the_title();
                ?>            
