...
/**
 * AIOVG_Premium_Vimeo_Import class.
 *
 * @since 2.6.2
 */
class AIOVG_Premium_Vimeo_Import {
	...
	public function get_videos() {
		require_once AIOVG_PLUGIN_DIR . 'premium/includes/vimeo-api/vimeo-api.php';

		// Vars
		$params = $this->get_params();

		$resource_type = sanitize_text_field( $params['type'] );
		$resource_id   = sanitize_text_field( $params['src'] );
		$api_user_id   = sanitize_text_field( $params['username'] );
		
		$args = array(
			'page'     => isset( $params['import_api_params']['page'] ) ? ( (int) $params['import_api_params']['page'] + 1 ) : 1,
			'per_page' => min( (int) $params['limit'], 100 )
		);

		if ( 'folder' == $resource_type ) {
			if ( isset( $params['filter_tag'] ) ) {
				$args['filter_tag'] = sanitize_text_field( $params['filter_tag'] );
			}

			if ( isset( $params['filter_tag_exclude'] ) ) {
				$args['filter_tag_exclude'] = sanitize_text_field( $params['filter_tag_exclude'] );
			}

			if ( isset( $params['include_subfolders'] ) ) {
				$args['include_subfolders'] = (int) $params['include_subfolders'];
			}
		}

		$no_of_iterations = ceil( (int) $params['limit'] / 100 );
		...
		for ( $i = 0; $i < $no_of_iterations; $i++ ) {
			if ( $i == $no_of_iterations - 1 ) { // Last iteration
				$args['per_page'] = (int) $params['limit'] - ( $i * $args['per_page'] );
			}

			$api  = new AIOVG_Premium_Vimeo_API( $resource_type, $resource_id, $api_user_id, $args );		
			$data = $api->get_feed();
			...
			if ( ! $data ) {
				...
				if ( is_wp_error( $error ) ) {
					$response->last_error = $error->get_error_message();
					break;
				}

				$response->last_error = __( 'No videos found matching your query.', 'all-in-one-video-gallery' );
				break;
			}
			...
			$response->params = array(
				'total_items' => (int) $api->get_total_items(),
				'page'        => (int) $api->get_page(),
				'has_ended'   => ( $api->has_ended() ? 1 : 0 )
			);
			...
			foreach ( $data as $video ) {
				...
				// Check in the excluded urls list
			...
			if ( $bypass_further_api_requests ) {
				$response->params['page'] = 0;
				$response->params['has_ended'] = 1;
		...
		if ( $response->success ) {
			...
			if ( $response->statistics['imported'] > 0 ) {
				...
				foreach ( $response->videos as $video ) {
					...
					if ( ! is_wp_error( $video_id ) ) {
						...
						$meta = array(
							'type'       => 'vimeo',
							'vimeo'      => aiovg_sanitize_url( $video->url ),
			...
			if ( ! empty( $params['schedule'] ) ) {
				if ( 'completed' != $response->status ) {
					if ( 'rescheduled' == $response->status && 0 == $response->params['has_ended'] ) {
	...
	public function before_delete_post( $video_id, $import_id ) {	
		$excluded_video = get_post_meta( $video_id, 'vimeo', true );
