...
class AIOVG_Uninstall {
	...
	public static function uninstall() {
		...
		if ( aiovg_fs()->is__premium_only() ) {
			// Delete all the premium custom post types
			$aiovg_post_types = array( 'aiovg_automations' );
			...
			// Delete all the premium terms & taxonomies
			$aiovg_taxonomies = array( 'aiovg_playlists' );
			...
			// Delete all the premium plugin options
			delete_option( 'aiovg_thumbnail_generator_settings' );
			delete_option( 'aiovg_automations_settings' );	
			delete_option( 'aiovg_playlists_settings' );		
			delete_option( 'aiovg_user_submission_settings' );
			delete_option( 'aiovg_antispam_settings' );
			delete_option( 'aiovg_user_account_settings' );
			delete_option( 'aiovg_email_video_pending_review_settings' );
			delete_option( 'aiovg_email_video_published_settings' );			
			delete_option( 'aiovg_seo_settings' );
			delete_option( 'aiovg_ads_settings' );

			// Delete the uploads directory created by the plugin
			require_once( ABSPATH . 'wp-admin/includes/class-wp-filesystem-base.php' );
			require_once( ABSPATH . 'wp-admin/includes/class-wp-filesystem-direct.php' );

			$upload_dir = wp_upload_dir();
			$dir = trailingslashit( $upload_dir['basedir'] ) . 'all-in-one-video-gallery';
