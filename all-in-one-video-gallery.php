 ...
 *
 * @wordpress-plugin
 * Plugin Name: All-in-One Video Gallery (Premium)
...
if ( function_exists( 'aiovg_fs' ) ) {
    aiovg_fs()->set_basename( true, __FILE__ );
    return;
}
if ( !function_exists( 'aiovg_fs' ) ) {
...
if ( aiovg_fs()->is__premium_only() ) {
     ...
     * @since 1.5.7
     */
    function aiovg_premium_run() {
        require_once AIOVG_PLUGIN_DIR . 'premium/init.php';
        $plugin = new AIOVG_Premium_Init();
        $plugin->run();
    }

    if ( aiovg_fs()->can_use_premium_code() ) {
        aiovg_premium_run();
    }
}
